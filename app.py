...
def latin():
    if request.method == "POST":
        return jsonify({"translation": latin_to_english(request.get_json())})

@app.route("/en_to_la", methods=["POST"])
def english():
    if request.method == "POST":
        return jsonify({"translation": english_to_latin(request.get_json())})

# takes part of speech and part of def ie. "noun:canis"
@app.route("/forms", methods=["POST"])
def forms():
    return jsonify({"forms": get_forms(request.get_json())})
